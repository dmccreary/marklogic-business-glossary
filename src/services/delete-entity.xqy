...
let $update :=
   if (exists($concept/skos:entity))
      then xdmp:node-replace($concept/skos:entity, <skos:entity>false</skos:entity>)
      else xdmp:node-insert-child($concept, <skos:entity>false</skos:entity>)

let $content :=
<div>
   The concept {$concept/skos:prefLabel/text()} is no longer an entity.
